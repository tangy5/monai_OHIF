...
class BasicInferTask(InferTask):
    ...
    def __call__(
        ...
        destination_folder = "./infer_files"
    
        # Ensure the destination folder exists
        os.makedirs(destination_folder, exist_ok=True)
        
        # Get the full destination path
        destination_file = os.path.join(destination_folder, os.path.basename(result_file_name))
        
        # Copy the file to the destination folder
        shutil.copy(result_file_name, destination_file)
    ...
    def check_prompts_format(self, label_prompt, points, point_labels):
        ...
        if label_prompt is None and points is None:
            # everything_labels = self.hyper_kwargs.get("everything_labels", None) # {"label_prompt":[1,3]}
            ...
            if everything_labels is not None:
                ...
                print('Innnnnnnnnnnnnnnnnnnnnn check:', label_prompt)
    ...
    def run_inferer(self, data: Dict[str, Any], convert_to_batch=True, device="cuda"):
        ...
        if network:
            ...
            # label_prompt = [torch.tensor(l) if not isinstance(l, torch.Tensor) else l for l in label_prompt]
            if label_prompt is not None:
            ...
            if points is not None:
                ...
                points = self.transform_points(
                    points, np.linalg.inv(data['image_meta_dict']["affine"]) @ data['image_meta_dict']["original_affine"]
                )
                points = torch.from_numpy(points).to(torch.device(device))
